...
// =============================================================================
//
// Processes completed debug sessions and generalizes accumulated knowledge.
//
// Pipeline:
...
export async function evolveFromTrace(
  ...
  try {
    ...
    for (const rule of newRules) {
      console.log(
        `    - ${rule.name} (from ${rule.derivedFrom.length} entries)`,
      );
    }
...
export async function evolveBatch(): Promise<EvolveResult[]> {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(
    `Batch complete: ${successful}/${results.length} traces processed`,
  );
  console.log(
