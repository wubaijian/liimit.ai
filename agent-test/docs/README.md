# Agent Prompts — Platformer GDD Architecture
...
```
User Prompt -> Confirm Platformer -> Scaffold -> GDD -> Assets -> Config -> Code -> Verify
                  |           |          |                          |        |
          (fixed mode)   (copy)   (generate-gdd reads         (read ALL  (debug
...
├── gdd/
│   └── core.md            # Platformer GDD format structure
└── modules/
    └── platformer/
...
| `core.md`           | generate-gdd sub-model | Phase 2 | Platformer GDD format and structure                   |
| `platformer.md`     | Main agent             | Phase 5 | Lifecycle, exact types, API signatures, code mistakes |
