...
5. Search `scene.start(` - verify all target scene keys are registered in main.ts
6. Search `import {` without `type` - verify interfaces use `type` keyword
7. Search `import { type` - verify every imported type name exists in the source file
8. Search `override` - verify every overridden method exists in the base class and visibility matches
9. Search `LEVEL_ORDER` - verify first entry matches the actual first game scene
...
| -------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------- |
| `npm install` / `npm ci`               | liimit.ai prepares the pinned lockfile before Agent startup | Stop and report "liimit.ai project dependency preparation failed" |
| `npm audit fix`                        | Security issues are not game logic errors                   | Focus on the actual error                                         |
| `npm update` or edit dependency ranges | Breaks the fixed template dependency contract               | Keep `package.json` and `package-lock.json` unchanged             |
| Delete `node_modules`                  | Removes dependencies managed by liimit.ai                   | Report the missing/corrupt dependency state                       |
| Reinstall packages                     | Bypasses the controlled Desktop dependency service          | Check import paths; report dependency preparation failures        |
...
| Run `npm install`, `npm ci`, or `npm update`  | Bypasses liimit.ai's pinned dependency preparation | Report dependency preparation failure; do not mutate dependencies |
