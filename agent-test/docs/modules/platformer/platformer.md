...
The fixed liimit.ai product keeps `LevelManager.ts` on `Level1Scene`. The
playable scene is `VisualLevelScene`, and its geometry comes from the
shared visual campaign in `src/levels.json`. The liimit.ai editor owns normal
interactive writes, while the Agent may make a user-requested, local campaign
edit. Both must preserve all unrequested levels and objects. Never replace this
contract with hardcoded coordinates or runtime-only obstacle injection.
...
The fixed entry imports `VisualLevelScene` and starts it through
`scene: [VisualLevelScene]`. Keep that registration so Web preview continues
to read the visual editor result. Do not replace it with generated level
scenes.
...
const config = {
  // Other fixed Phaser settings remain here.
...
- [ ] `LevelManager.ts` still contains only `Level1Scene`
- [ ] `main.ts` still imports and starts `VisualLevelScene`
- [ ] `VisualLevelScene` key remains `Level1Scene`
- [ ] `src/levels.json` changes only for the requested level and remains valid
- [ ] legacy `src/level.json` remains unchanged
- [ ] manual and automated playtesting use the same campaign objects
