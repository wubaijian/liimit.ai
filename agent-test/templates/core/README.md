...
Projects created by liimit.ai use the committed `package-lock.json`. The desktop
app prepares these pinned dependencies through its controlled dependency
service before the Agent starts; ordinary users do not need to open a terminal
or run an install command.

When using this template independently of liimit.ai, install exactly the locked
