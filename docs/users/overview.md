...
- **Prompt → playable platformer**: describe a side-scrolling game in plain language; liimit.ai loads its fixed Phaser 3 platformer template, writes the code, and tests it.
- **Fixed Game Skill**: a stable platformer scaffold plus a systematic Debug Protocol keeps designs within the code capabilities that actually ship.
...
- **Headless / scriptable**: the inherited CLI can run the same fixed platformer workflow for development and verification.
