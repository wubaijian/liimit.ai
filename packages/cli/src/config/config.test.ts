...
describe('loadCliConfig with allowed-mcp-server-names', () => {
  ...
  it('should allow no settings or extension MCP servers if the flag is provided but empty', async () => {
    process.argv = ['node', 'script.js', '--allowed-mcp-server-names', ''];
    const argv = await parseArguments({} as Settings);
    const extensions: Extension[] = [
      {
        path: '/path/to/fixed-mode-test-extension',
        config: {
          name: 'fixed-mode-test-extension',
          version: '1.0.0',
          mcpServers: {
            'extension-server': { url: 'http://localhost:8083' },
          },
        },
        contextFiles: [],
      },
