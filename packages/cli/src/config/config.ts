...
import { getDesktopCredentials } from './desktopCredentials.js';
...
export async function loadCliConfig(
  ...
  const desktopCredentials = getDesktopCredentials();
  ...
  if (desktopCredentials?.mcpServers) {
    mcpServers = { ...mcpServers, ...desktopCredentials.mcpServers };
  }
  ...
  const apiKey =
    (selectedAuthType === AuthType.USE_OPENAI
      ? argv.openaiApiKey ||
        desktopCredentials?.main.apiKey ||
  ...
  const baseUrl =
    (selectedAuthType === AuthType.USE_OPENAI
      ? argv.openaiBaseUrl ||
        desktopCredentials?.main.baseUrl ||
        process.env['OPENAI_BASE_URL'] ||
        settings.security?.auth?.baseUrl
      : '') || '';
  const resolvedModel =
    argv.model ||
    desktopCredentials?.main.model ||
    (selectedAuthType === AuthType.USE_OPENAI
  ...
  const openGameProviders = desktopCredentials
    ? {
        ...(settings.openGame?.providers ?? {}),
        ...desktopCredentials.providers,
      }
    : settings.openGame?.providers;
  const runtimeRequestTimeout = boundedEnvironmentInteger(
    'MODEL_REQUEST_TIMEOUT',
    30_000,
    30 * 60_000,
  );
  const runtimeMaxRetries = boundedEnvironmentInteger(
    'MODEL_MAX_RETRIES',
    0,
    5,
  );
  ...
  return new Config({
    ...
    generationConfig: {
      ...
      ...(runtimeRequestTimeout !== undefined
        ? { timeout: runtimeRequestTimeout }
        : {}),
      ...(runtimeMaxRetries !== undefined
