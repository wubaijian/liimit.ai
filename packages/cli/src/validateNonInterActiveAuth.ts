...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    if (!useExternalAuth) {
      const desktopOpenAiCredential =
        authType === AuthType.USE_OPENAI &&
        !process.env['OPENAI_API_KEY'] &&
        Boolean(nonInteractiveConfig.getContentGeneratorConfig?.()?.apiKey);
      const err = desktopOpenAiCredential
        ? null
        : validateAuthMethod(String(authType));
      if (err != null) {
        throw new Error(err);
      }
    }
