...
export class OpenAICompatImageService extends BaseService {
  ...
  async generateImage(
    ...
    // Some OpenAI-compatible providers (e.g. aihubmix gpt-image-2) reject
    // the `response_format` parameter outright, and every supported
    // provider returns either `url` or `b64_json` by default, so it is
    // omitted unless explicitly requested via env var. Both response
    // shapes are handled below.
    if (process.env['OPENGAME_IMAGE_RESPONSE_FORMAT_URL'] === '1') {
...
export function createImageService(config: ImageModelConfig): IImageService {
  switch (config.modelType) {
    ...
    default:
      throw new Error(
        `Provider ${config.modelType} does not support image generation`,
      );
  }
}
