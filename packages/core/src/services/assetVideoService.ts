...
export class FrameExtractionService extends BaseService {
  ...
  async extractFramesLocal(
    ...
    frames: Array<{ frameIndex: number; path: string; url: string }>;
    frameUrls: string[];
    framePaths: string[];
    videoPath: string;
  }> {
    ...
    const samples: Array<{ frameIndex: number; timestamp: number }> = [];

    if (firstLastOnly || frameCount <= 2) {
      samples.push({ frameIndex: 0, timestamp: fromTime });
      if (frameCount > 1) {
        samples.push({ frameIndex: frameCount - 1, timestamp: toTime - 0.1 });
      }
      ...
      for (let i = 0; i < frameCount; i++) {
        samples.push({ frameIndex: i, timestamp: fromTime + i * interval });
      }
    }

    this.log(
      `Extracting at timestamps: ${samples.map(({ timestamp }) => timestamp.toFixed(2)).join('s, ')}s`,
    );

    const frames: Array<{ frameIndex: number; path: string; url: string }> = [];
    const ffmpegExe = await this.getFFmpegPath();
    if (!ffmpegExe) throw new Error('FFmpeg not found');

    for (let i = 0; i < samples.length; i++) {
      const { frameIndex, timestamp } = samples[i];
      ...
      try {
        ...
        this.log(
          `Extracted frame ${i + 1}/${samples.length} at ${timestamp.toFixed(2)}s`,
        );
      } catch (error) {
    ...
    this.log(`Successfully extracted ${frames.length} frames`);
...
export function createVideoService(config: VideoModelConfig): IVideoService {
  switch (config.modelType) {
    ...
    default:
      throw new Error(
        `Provider ${config.modelType} does not support video generation`,
      );
  }
}
