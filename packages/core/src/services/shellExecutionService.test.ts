...
describe('getSanitizedShellEnvironment', () => {
  it('removes provider credentials while preserving normal runtime variables', () => {
    const originalPath = process.env['PATH'];
    const originalOpenAIKey = process.env['OPENAI_API_KEY'];
    const originalReasoningKey = process.env['OPENGAME_REASONING_API_KEY'];
    const originalGithubToken = process.env['GITHUB_TOKEN'];

    process.env['OPENAI_API_KEY'] = 'main-secret';
    process.env['OPENGAME_REASONING_API_KEY'] = 'reasoning-secret';
    process.env['GITHUB_TOKEN'] = 'github-secret';
    process.env['GAMEAGENT_SAFE_TEST'] = 'visible';

    try {
      const env = getSanitizedShellEnvironment({ TERM: 'xterm-256color' });
      expect(env['OPENAI_API_KEY']).toBeUndefined();
      expect(env['OPENGAME_REASONING_API_KEY']).toBeUndefined();
      expect(env['GITHUB_TOKEN']).toBeUndefined();
      expect(env['GAMEAGENT_SAFE_TEST']).toBe('visible');
      expect(env['TERM']).toBe('xterm-256color');
      expect(env['PATH']).toBe(originalPath);
    } finally {
      if (originalOpenAIKey === undefined) delete process.env['OPENAI_API_KEY'];
      else process.env['OPENAI_API_KEY'] = originalOpenAIKey;
      if (originalReasoningKey === undefined) {
        delete process.env['OPENGAME_REASONING_API_KEY'];
      } else {
        process.env['OPENGAME_REASONING_API_KEY'] = originalReasoningKey;
      }
      if (originalGithubToken === undefined) delete process.env['GITHUB_TOKEN'];
      else process.env['GITHUB_TOKEN'] = originalGithubToken;
      delete process.env['GAMEAGENT_SAFE_TEST'];
    }
  });
});
