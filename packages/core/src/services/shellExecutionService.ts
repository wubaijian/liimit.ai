...
/**
 * Agent provider credentials belong to the runtime process, not to commands
 * generated inside a project. Passing the full runtime environment to a shell
 * would allow an npm script (or a prompt-injected command) to print or exfiltrate
 * every configured model key.
 */
export function getSanitizedShellEnvironment(
  overrides: NodeJS.ProcessEnv = {},
): NodeJS.ProcessEnv {
  const sanitized: NodeJS.ProcessEnv = {};
  for (const [name, value] of Object.entries(process.env)) {
    if (value !== undefined && !SECRET_ENV_SEGMENT.test(name)) {
  ...
  private static childProcessFallback(
    ...
    try {
      ...
      const child = cpSpawn(commandToExecute, [], {
        ...
        detached: !isWindows,
        env: getSanitizedShellEnvironment({
          QWEN_CODE: '1',
          TERM: 'xterm-256color',
          PAGER: 'cat',
        }),
