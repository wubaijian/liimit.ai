...
> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    // Old callers may still send the retired multi-archetype `mode` field even
    // though it is no longer part of the public TypeScript or JSON schema.
    // Reject it before resolving the workspace or creating any files.
    if (Object.prototype.hasOwnProperty.call(this.params, 'mode')) {
      return {
        llmContent:
          'Error: generate_tilemap 仅支持横版平台跳跃地图；请使用 legend 明确声明空气和实体平台。',
        returnDisplay: 'Map Generation Failed: Unsupported retired mode',
        error: {
          message:
            'generate_tilemap 仅支持横版平台跳跃地图，不再接受 mode 参数',
        },
      };
    }
    ...
    const autoTileChars = this.params.auto_tile_chars || ['#'];
...
> {
  ...
  constructor(private config: Config) {
    super(
      ...
      'Converts side-view platformer ASCII layouts into Phaser Tilemap JSON with blob autotiling (bitmasking).',
      Kind.Execute,
      {
        type: 'object',
        properties: {
          ...
          maps: {
            type: 'array',
            description:
              'Array of side-view platformer map definitions. Each map uses one tileset and an explicit character legend.',
            items: {
              type: 'object',
              properties: {
                map_key: { type: 'string' },
              ...
              required: ['map_key', 'layout_ascii', 'legend'],
            },
          },
          map_key: { type: 'string' },
          layout_ascii: { type: 'array', items: { type: 'string' } },
