...
export function parse(yamlString: string): Record<string, unknown> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Agent Skills commonly use folded/literal YAML block scalars for long
    // descriptions. They are top-level frontmatter fields, so support that
    // standards-compatible shape without turning this minimal parser into a
    // general-purpose YAML implementation.
    const blockScalar = line.match(/^([^\s][^:]*):\s*([>|])[-+]?\s*(?:#.*)?$/);
    if (blockScalar) {
      if (inArray) {
      ...
      result[blockScalar[1].trim()] = parseBlockScalar(
        contentLines,
        blockScalar[2],
      );
      i = next - 1;
...
function parseBlockScalar(lines: string[], style: string): string {
  ...
  if (style === '|') return values.join('\n').trim();

  let folded = '';
  for (const value of values) {
    if (!value) {
      folded = `${folded.trimEnd()}\n`;
    } else {
